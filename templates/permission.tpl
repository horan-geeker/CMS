...
<ul>
	<li><a href="permission.php?action=show">权限列表</a></li>
...
<table class="tb" cellspacing="0">
	<tr><th>编号</th><th>名称</th><th>权限描述</th><th>操作</th></tr>
	{if $allPermission}
	...
	<tr>
		...
		<td>{@value->name}</td>
		<td>{@value->info}</td>
		<td><a href="permission.php?action=update&id={@value->id}">修改</a> |
		 <a href="permission.php?action=delete&id={@value->id}" onclick="return confirm('确定要删除吗？')">删除</a></td>
	</tr>
	{/foreach}
	{else}
...
<table cellspacing="0" class="tb left">
	...
	<tr><td><span class="level_info">权限描述：</span><textarea name="info"></textarea></td></tr>
	<tr>
    	<td>
    		<input type="submit" name="send" value="新增权限" class="submit" onclick="return checkAddForm();" /> [ <a
					href="level.php?action=show">返回列表</a> ]
		</td>
	</tr>
</table>
</form>
